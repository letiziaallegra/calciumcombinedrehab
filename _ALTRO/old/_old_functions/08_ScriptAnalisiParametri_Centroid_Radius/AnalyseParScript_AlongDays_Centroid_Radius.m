...
dirAllDataGCamp = ['C:\Users\',Username,'\Google Drive\Piattaforma Stefano\LENS_SSSA\ELABORAZIONE DATA\_data_MAT_GCamp'];
%list animals
...
%%%
%%% ROIs Centroid based
ROINameToTakeCentroid = {'Main ROI'};
ROIToTakeCentroid     = [1];
NumROICentroid        = length(ROIToTakeCentroid);
ColorsStartCentroid   = [0 0 0.5; 0 0.5 0; 0.5 0 0];
%%%
...
switch SelROI
    ...
    case 1
        %ROI to take
        ROINameToTake = ROINameToTakeCentroid;
        NumROI        = NumROICentroid;
        ROIToTake     = ROIToTakeCentroid;
        ColorsStart   = ColorsStartCentroid;
        ListROIs      = ones(NumROICentroid,1);
    case 2
        %ROI to take
        ROINameToTake = [ROINameToTakeManual, ROINameToTakeCentroid];
        NumROI        = NumROIManual+NumROICentroid;
        ROIToTake     = [ROIToTakeManual ROIToTakeCentroid];
        ListROIs      = [zeros(NumROIManual,1); ones(NumROICentroid,1)];
end
...
%         NameFile_GCamp_Day  = ['dataMouseGCamp_',NameAnimal,'_',NameDay,'_Par'];  
        NameFile_GCamp_Day  = ['dataMouseGCamp_',NameAnimal,'_',NameDay,'_Par_MainCentroid_Par']; 
        ...
        if exist([Path_GCamp_Day,'\',NameFile_GCamp_Day,'.mat']) %if exist
            ...
            if isfield(dataGCamp,'MainCentroid_PeaksPar_Fx_Fluo') %if PeaksPar_Fx_Fluo
                ...
                switch SelROI
                    ...
                    case 1
                        %ROI Centroid based
                        NumSig        = size(dataGCamp.MainCentroid_PeaksPar_Fx_Fluo,2);
                    case 2
                        %ROI + ROI Centroid Based
                        NumSig        = size(dataGCamp.PeaksPar_Fx_Fluo,2) + size(dataGCamp.MainCentroid_PeaksPar_Fx_Fluo,2);
                end
                ...
                for ip=1:NumPar  %if NumSig
                    ...
                    for iR=1:NumROI
                        ...
                        elseif ListROIs(iR) == 1
                            %Force Par
                            ForcePar = dataGCamp.MainCentroid_PeaksPar_Fx_Fluo{1,Curr_ROI}(:,Curr_Par);
                            %Fluo Par
                            FluoPar  = dataGCamp.MainCentroid_PeaksPar_Fx_Fluo{2,Curr_ROI}(:,Curr_Par);
                        end
