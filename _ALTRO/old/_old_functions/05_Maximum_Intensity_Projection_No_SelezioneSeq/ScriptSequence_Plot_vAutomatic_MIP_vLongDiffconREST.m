...
%     MIP_tech_List = {'sum'};
    ...
    for iMT=1:length(MIP_tech_List)
        ...
        for nd_i=3:length(NumDaysFolder)
            ...
            if  ~isempty(SeqDir) && ~isempty(SeqDir_REST)
                ...
                %%% Dir where saving
                DirDayToSaveMIP = [MainDir,nday_date,'\','MIP_Long_Diff_Rest'];
                ...
                for int=1:NumTrialsUsed
                    ...
                    MIP_all = [];
                    for inf=1:NumFrames
                        MIP_all(:,:,inf) = MatrixImageSequence{int,:}(:,:,inf);
                    end
                    ...
                    elseif strcmp(MIP_tech,'sum')
                        ...
                        if int==NumTrialsUsed
                            clear MIP;
                            MIP_TASK    = sum(SIP_Store_ALL,3);
                        end
                    end   
                    
                end
                clear MIP_Store_ALL SIP_Store_ALL
                
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %%%%%%%%% REST  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                MatrixImageSequence_REST = ImageSequence_REST.MatrixImageSequence;
                NumTrials = size(ImageSequence_REST,1);
                TrialsUsed    = [1:NumTrials];
                % TrialsUsed    = [1];
                NumTrialsUsed = length(TrialsUsed);
                rw        = size(MatrixImageSequence_REST{1,1},1);
                cl        = size(MatrixImageSequence_REST{1,1},2);
                if isfield(ImageSequence,'MinSize')
                ...
                MIP_REST_Store_ALL = zeros(rw, cl, NumFrames);
                SIP_REST_Store_ALL = zeros(rw, cl, NumFrames);
                
                for int=1:NumTrials
                    
                    MIP_all = [];
                    for inf=1:NumFrames
                        MIP_all(:,:,inf) = MatrixImageSequence_REST{int,:}(:,:,inf);
                    end
                    ...
                    if strcmp(MIP_tech,'max')
                        %MAX
                        MIP_REST_Store_ALL(:,:,int) = MIP;
                        
                        if int==NumTrialsUsed
                            clear MIP;
                            MIP_REST    = max(MIP_REST_Store_ALL,[],3);
                        end
                    
                    elseif strcmp(MIP_tech,'sum')
                        %%SUM
                        SIP_REST_Store_ALL(:,:,int) = MIP;
                        
                        if int==NumTrialsUsed
                            clear MIP;
                            MIP_REST    = sum(SIP_REST_Store_ALL,3);
                        end
                    end
                ...
                clear MIP_REST_Store_ALL SIP_REST_Store_ALL          
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                
                %%%% Difference %%
                MIP = MIP_TASK-MIP_REST;
                %%%%%%%%%%%%%%%%%%
