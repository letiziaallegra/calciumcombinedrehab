...
% Animal_name = 'GCaMP3_control';
% Animal_name = 'GCaMP4_control';
% Animal_name = 'GCaMP9_stroke';
% Animal_name = 'GCaMP10_stroke';
% Animal_name = 'GCaMP11_stroke';
% Animal_name = 'GCaMP14_stroke';
% Animal_name = 'GCaMP15_stroke';
...
MainDir = [UsbPort,':\LENS\Animals Data\NoBregmaREF\',Animal_name,'\'];
NumDaysFolder = dir(MainDir);


%%%%%%%% load Reference File %%%%%%%%%%%%%%%%%%%%%%%%
RefDir      = ['C:\Users\',UserName,'\Google Drive\Piattaforma Stefano\LENS_SSSA\ELABORAZIONE DATA\Script_Flip_Find_References\MAT_Rot_Trans_NoBregmaREF'];
...
for iMT=1:length(MIP_tech_List)
    ...
    for nd_i=3:length(NumDaysFolder)
        ...
        if  ~isempty(SeqDir)
            ...
            %%% Dir where saving
            DirDayToSaveMIP = [MainDir,nday_date,'\','MIP'];
...
%             end
            ...
            H_MIP_Fig = figure('Name',['Image MIP around Force Peak Onset day_',nday]);
            subplot(221)
            imagesc(MIP)
            colormap gray
            ...
            if 1             
                ...
                if trans_ud<0
                    ...
                    trans_ud = -1;
                elseif trans_ud>0
                    MIP_OR_2(:,trans_X:end) = MIP_OR(:,1:cl-trans_X+1);
                    trans_ud = +1;
                elseif trans_ud == 0
                    MIP_OR_2 = MIP_OR;
                    trans_ud = 0;
                ...
                %new coordinates of Bregma -> FAKE (perchè non so dove è)
                XbN = 1;
                YbN = 29; %==0.250 mm
            ...
            %
            H_MIP_Fig_filename = [Animal_name,'_',MIP_tech,'MIP_Fig_',nday];
    ...
    MIP_All_Day_Fig_filename_week =  [Animal_name,'_',MIP_tech,'MIP_Overlapped_ROI_Fig_'];
    ...
    %save Im_med mat
    save([MainDir,'\Im_AlongDays_',Animal_name,'_MIP_',MIP_tech],'Im_med')
    ...
    if size(rot_transl,1)>5
        ...
        for wee_i=1:size(wee,1)
            ...
            MIP_All_Day_Fig_filename_week =  [Animal_name,'_',MIP_tech,'MIP_Week_',num2str(wee_i),'_Overlapped_ROI_Fig_'];
            MIP_All_Day_Fig = figure('Name',MIP_All_Day_Fig_filename_week);
            imagesc(Im_med)
            hold on
            text(XbN,YbN, 'X','Color','red','FontSize',10);
            ...
            save([MainDir,'\Im_AlongDays_Week_',num2str(wee_i),'_',Animal_name,'_MIP_',MIP_tech],'Im_med')
