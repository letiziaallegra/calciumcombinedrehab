...
for nd_i=3:length(NumDaysFolder)
    ...
    if exist([MainDir,'\',DayCurrDir,'\',filename])
        ...
        if isfield(dataGCamp,'SpeedPeak')
            display('Check OK')
        else
            error('SpeedPeak missing')
        end
    else
    ...
    %cerca i first mov indices
    for i_s=1:size(SpeedPeak,1)
        
        i_maxSp = SpeedPeak(i_s,4)*Fs;
        
        interval_Acc = i_maxSp-50:i_maxSp;
        Acc_Buf = Acc(interval_Acc);
        
        [FMov, i_FMov] = findpeaks(-Acc_Buf);
        
        sec_FirstMov = interval_Acc(i_FMov)/Fs;
        List_i_FirstMov = [List_i_FirstMov; sec_FirstMov];
    ...
    Folder_Seq_Name = [MainDir,'\',DayCurrDir,'\','Sequence_FirstMov'];
    ...
    Filename = ['ImageSequence_',dataGCamp.Info.Name,'_',dataGCamp.Info.Date,'_FirstMov_ROI_',num2str(ROISelected)];
