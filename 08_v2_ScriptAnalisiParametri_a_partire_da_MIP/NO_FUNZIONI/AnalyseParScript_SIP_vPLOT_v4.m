...
for ipar=3:14
    ...
    load('dataMouseGCamp_MEAN_SelROI_1_27-Sep-2016_15-17')
...
elseif SelROI == 1
    ...
    Folder_Where_Save = '_Folder_Image_And_Stat_ParScript';
    %% ROI of interest
    ROI_List = [1 2 3];
    %%%
elseif SelROI == 2
            ...
            if ~isempty(na_inDay)
                ...
                if sum(ismember(ListDaysToTake,nd))>0
                    ...
                    if ~isempty(MeanPar{ParForce,nd}{na_inDay,1})
                        ...
                        MeanFluo  = [MeanFluo;  MeanPar{ParFluo,nd}{na_inDay,1}(ROI,3)];
                        StdFluo   = [StdFluo;   MeanPar{ParFluo,nd}{na_inDay,1}(ROI,4)];
...
%                                     MeanForceTot(:,tr+1) = nanmean(MeanForce_Buf(:,TreatMouseTot==tr),2);
                ...
                Filename = {'control','stroke','rehab 4w','rehab 1w'};
...
%                 legend(Filename)
                ...
                xlswrite(['DataForceStat_',ExcelStr,'_',ParNameToPlot{ParForce},'_',ROINameToPlot{nroi}],ExcelSaveForce ,tr)
                xlswrite(['DataFluoStat_',ExcelStr,'_',ParNameToPlot{ParForce},'_',ROINameToPlot{nroi}],ExcelSaveFluo ,tr)
                %
