...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [InfoPicchiForzeBuffer InfoPicchiVelocita  delToT] = ForcePeaksCleaner_FromSpeed(Inf, Inf_Speed,Fs, Threshold, Threshold_Speed)

InfoPicchiForzeBuffer = InfoPicchiForza;
delToT = [];
InfoPicchiVelocitaOLD = InfoPicchiVelocita; %%%TOGLIERE

num_SpeedPeaks = size(Inf_Speed,1);
num_ForcePeaks = size(Inf,1);
ForcePeaks_OK_NO = ones(num_ForcePeaks,1);
...
for ns=1:num_SpeedPeaks
    ...
    St_Sp = Inf_Speed(ns,1)*Fs;
    En_Sp = (Inf_Speed(ns,1)+Inf_Speed(ns,2))*Fs-1;
    Interval_Sp = [St_Sp:En_Sp]';
    
    for nf=1:num_ForcePeaks
        %Interval of the Force Peak
        St_F = Inf(nf,1)*Fs;
        En_F = (Inf(nf,1)+Inf(nf,2))*Fs-1;
        
        %member or not
        memb_St(:,St_F) = ismember(Interval_Sp,St_F);
        memb_En(:,En_F) = ismember(Interval_Sp,En_F);
