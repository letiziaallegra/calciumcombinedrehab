...
for i=1:length(ListAnimalTogether)
    ...
    folderAn = ListAnimalTogether{i};
        ...
        if strfind(folderAn,'stroke_BoNT')
            ...
            MIP_sum_REHAB1  = MIP_sum_REHAB1+double(fileA.Im_med/max(max(fileA.Im_med)));
            clear fileA
            %rehab 4
            fileA = load( [folder,'\',folderAn,'\','Im_AlongDays_Week_4_',folderAn,'_Long_SelSeq_SIP']);
            MIP_sum_REHAB4  = MIP_sum_REHAB4+double(fileA.Im_med/max(max(fileA.Im_med)));
            clear fileA
        ...
        elseif strfind(folderAn,'stroke')
            ...
            MIP_sum_STROKE  = MIP_sum_STROKE+double(fileA.Im_med/max(max(fileA.Im_med)));
            clear fileA
        ...
        elseif strfind(folderAn,'control')
            ...
            fileA = load( [folder,'\',folderAn,'\','Im_AlongDays_',folderAn,'_Long_SelSeq_SIP']);
            MIP_sum_CONTROL  = MIP_sum_CONTROL+double(fileA.Im_med/max(max(fileA.Im_med)));
            clear fileA
