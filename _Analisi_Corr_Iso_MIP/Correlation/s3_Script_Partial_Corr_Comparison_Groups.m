...
TASK_FOLDER = [HD,':\LENS\Correlazioni_Parziali_SelSeq_MacroAree\Partial_Corr_Areas_SEPWEEK_PART',PART{1}];
REST_FOLDER = [HD,':\LENS\Correlazioni_Parziali_SelSeq_MacroAree\Partial_Corr_Areas_SEPWEEK_PART',PART{2}];
...
AREA_NAME   = {'Motor','CFA','Sensory','Caudal'};
TREAT_NAME = {'C_T' 'S_T' 'R1w_T' 'R4w_T' 'C_R' 'S_R' 'R1w_R' 'R4w_R'};
...
for i_P=1:size(MATRIX_INFO_M1,1) %PART   
    ...
    for i_T=1:size(MATRIX_INFO_M1,2) %TREAT
        ...
        hold on
        plot(1:length(AREA_NAME),Med3,'color',COLOR_T(i_T,:),'Marker','o')
...
filename_1 = ['Average_Partial_Corr_with_CFA_among_groups',R_name,'_FIG'];
...
filename_1 = ['Median_Partial_Corr_with_CFA_among_groups',R_name,'_FIG'];
saveas(gca,[filename_1,ExtraLabel],'fig');
saveas(gca,[filename_1,ExtraLabel],'jpeg');
