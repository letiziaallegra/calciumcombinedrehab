...
for i_selPART = 1:length(SEL_PART_LIST) %for SEL_PART_LIST 
...
%%%%%%%% Where SAVE Folder %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% PER FARLI INSIEME %%%%
Main_SAVE_Dir = [UsbPort,':\LENS\Correlazioni_Parziali_SelSeq_MacroAree\'];
SEL_SEP = 'DAY';
SAVE_Dir      = [Main_SAVE_Dir,'Partial_Corr_Areas_','SEP',SEL_SEP,'_PART',SEL_PART];
SAVE_Dir_Method1 = [SAVE_Dir,'\Method1\'];
SAVE_Dir_Method2 = [SAVE_Dir,'\Method2\'];
...
%     FuncAres_Name = {'M2','CFA','M1Sh','M1HL','M1Tk','S1','S1Bc','RSD','LPTa','V1','AuD'};
    load('Region_MACRO_Area_FunctionalMask_Index'); %-> RegionArea_FunctionalMask_Index
    FuncAres_Name = {'MotorAreas','CFA','Sensory','Caudal'};
...
%             load([DirDay,'\',SeqDir(3,1).name])
            ...
            elseif exist([DirDay,'\','ImageSequence_',Animal_name,'_',nday,'_Status_5  6  7_ROI_1.mat'])
                load([DirDay,'\','ImageSequence_',Animal_name,'_',nday,'_Status_5  6  7_ROI_1']);
...
%             if strcmp(SEL_SEP,'DAY') %if SEL_SEP save Fig
                ...
                if NumTrialsUsed>0 %there are Trials ok
                    
                    %%% CORRELAZIONE %%%
                    [Mat_corr_1 Mat_corr_2 Mat_corr_3] = Fun_Compute_Partial_Corr(FuncAreaValue_Day_Met1, FuncAreaValue_Day_Met2, FuncAreaValue_Day_Met3, Num_FunAreas);
                    
                else %there are no Trials ok
...
%             elseif strcmp(SEL_SEP,'WEEK')
                ...
                %%%%numero di punti nella sequenza del giorno corrente deve essere uguale a quella dei giorni precedenti (si prende la dimensione più piccola)
                %%% per Method 1 e 2
                diff_Frames = NumFramesPrevDay - NumFrames;
                if diff_Frames > 0
                    FuncAreaValue_Week_Met1 = FuncAreaValue_Week_Met1(1:NumFrames,:);
                ...
                if sum(str2num(nday) == weekly_day_stop)>0
                    ...
                    %%%%%%%%%%%%%%%%%%%%%%%%%%
                    %%% CORRELAZIONE %%%
                    [Mat_corr_1_W Mat_corr_2_W Mat_corr_3_W] = Fun_Compute_Partial_Corr(FuncAreaValue_Week_Met1, FuncAreaValue_Week_Met2, FuncAreaValue_Week_Met3, Num_FunAreas);
