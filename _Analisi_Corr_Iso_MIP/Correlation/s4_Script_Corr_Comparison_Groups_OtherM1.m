...
%             if strfind(AnimalName,'GCaMP') &&  ~strcmp(AnimalName,'GCaMP16_stroke_BoNT') &&  ~strcmp(AnimalName,'GCaMP17_stroke_BoNT') &&  ~strcmp(AnimalName,'GCaMP16_stroke') && ~strcmp(AnimalName,'GCaMP11_stroke_BoNT') %if GCAMP
            if strfind(AnimalName,'GCaMP') &&  ~strcmp(AnimalName,'GCaMPChR2_16_stroke_BoNT') &&  ~strcmp(AnimalName,'GCaMP_17_stroke_BoNT') %if GCAMP
...
AREA_NAME         = {'M1Sh','M1HL','Sensory','V1'};
AREA_NAME_index   = {3, 4, [6 7], 10};
% AREA_NAME         = {'otherM1','S1','S1Bc','RSD','V1'};
% AREA_NAME_index   = {[3 4 5], 6, 7, 8, 10};
% AREA_NAME         = {'otherM1','Sensory','RSD','V1'};
% AREA_NAME_index   = {[3 4 5], [6 7], 8, 10};
AreaOfInterest_OK = [];
TREAT_NAME = {'C_T' 'S_T' 'R1w_T' 'R4w_T' 'C_R' 'S_R' 'R1w_R' 'R4w_R'};
...
for i_P=1:size(MATRIX_INFO_M1,1) %PART   
    ...
    for i_T=1:size(MATRIX_INFO_M1,2) %TREAT
        ...
        MATRIX_INFO_M2_Buf = [];
        MATRIX_INFO_M3_Buf = [];
        for i_AN = 1:length(AREA_NAME_index)
            MATRIX_INFO_M1_Buf = [MATRIX_INFO_M1_Buf; nanmean(MATRIX_INFO_M1{i_P,i_T}(AREA_NAME_index{i_AN},:),1)];
            MATRIX_INFO_M2_Buf = [MATRIX_INFO_M2_Buf; nanmean(MATRIX_INFO_M2{i_P,i_T}(AREA_NAME_index{i_AN},:),1)];
            MATRIX_INFO_M3_Buf = [MATRIX_INFO_M3_Buf; nanmean(MATRIX_INFO_M3{i_P,i_T}(AREA_NAME_index{i_AN},:),1)];
        end
        
        
        Med1 = nanmean(MATRIX_INFO_M1_Buf,2);
        SDE1 = nanstd(MATRIX_INFO_M1_Buf,[],2)/sqrt(size(MATRIX_INFO_M1_Buf,2));
        Med1(AreaOfInterest_OK) = NaN;
        subplot(2,length(METHOD),i_P+INC)
        hold on
        errorbar(Med1,SDE1,'Color',COLOR_T(i_T,:),'Marker','o')
        ...
        Med2 = nanmean(MATRIX_INFO_M2_Buf,2);
        SDE2 = nanstd(MATRIX_INFO_M2_Buf,[],2)/sqrt(size(MATRIX_INFO_M2_Buf,2));
        Med2(AreaOfInterest_OK) = NaN;
        subplot(2,length(METHOD),i_P+1+INC)
        hold on
        errorbar(Med2,SDE2,'color',COLOR_T(i_T,:),'Marker','o') 
        xlim([0.5 11.5])
        ...
        Med3 = nanmean(MATRIX_INFO_M3_Buf,2);
        SDE3 = nanstd(MATRIX_INFO_M3_Buf,[],2)/sqrt(size(MATRIX_INFO_M3_Buf,2));
        Med3(AreaOfInterest_OK) = NaN;
...
for i_P=1:size(MATRIX_INFO_M1,1) %PART   
    ...
    for i_T=1:size(MATRIX_INFO_M1,2) %TREAT
        ...
        MATRIX_INFO_M3_Buf = [];
        for i_AN = 1:length(AREA_NAME_index)
            MATRIX_INFO_M1_Buf = [MATRIX_INFO_M1_Buf; nanmedian(MATRIX_INFO_M1{i_P,i_T}(AREA_NAME_index{i_AN},:),1)];
            MATRIX_INFO_M2_Buf = [MATRIX_INFO_M2_Buf; nanmedian(MATRIX_INFO_M2{i_P,i_T}(AREA_NAME_index{i_AN},:),1)];
            MATRIX_INFO_M3_Buf = [MATRIX_INFO_M3_Buf; nanmedian(MATRIX_INFO_M3{i_P,i_T}(AREA_NAME_index{i_AN},:),1)];
        end
        
        
        Med1 = nanmedian(MATRIX_INFO_M1_Buf,2);
        SDE1 = nanstd(MATRIX_INFO_M1_Buf,[],2)/sqrt(size(MATRIX_INFO_M1_Buf,2));
        Med1(AreaOfInterest_OK) = NaN;
        subplot(2,length(METHOD),i_P+INC)
        hold on
        ...
        Med2 = nanmedian(MATRIX_INFO_M2_Buf,2);
        SDE2 = nanstd(MATRIX_INFO_M2_Buf,[],2)/sqrt(size(MATRIX_INFO_M2_Buf,2));
        Med2(AreaOfInterest_OK) = NaN;
        subplot(2,length(METHOD),i_P+1+INC)
        hold on
        ...
        Med3 = nanmedian(MATRIX_INFO_M3_Buf,2);
        SDE3 = nanstd(MATRIX_INFO_M3_Buf,[],2)/sqrt(size(MATRIX_INFO_M3_Buf,2));
        Med3(AreaOfInterest_OK) = NaN;
        subplot(2,length(METHOD),i_P+2+INC)
        hold on
