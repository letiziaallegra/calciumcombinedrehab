...
%     clearvars -except CurrDir UsbPortHD UserName AnimalDir ListAnimalTogether LATo
    ...
    ROI_Folder   = ['C:\Users\',UserName,'\Google Drive\Piattaforma Stefano\ELABORAZIONE DATA\05_Maximum_Intensity_Projection\_Store_MIP_all_Days_and_MAT\',Animal_Name];
    
    %%%%%%%%%%
    if strfind(Animal_Name,'BoNT')
        
        %week1
        load([ROI_Folder,'\','Im_AlongDays_Week_1_',Animal_Name,'_MIP_max']);
        Im_MIP = Im_med;
        Im_MIP = Im_MIP>=ThreshMIP;        
        clear Im_med
        load([ROI_Folder,'\','Im_AlongDays_Week_1_',Animal_Name,'_MIP_sum']);
        Im_SIP = Im_med;
        Im_SIP = Im_SIP>=ThreshMIP;
        clear Im_med
                
        %week4
        load([ROI_Folder,'\','Im_AlongDays_Week_4_',Animal_Name,'_MIP_max']);
        Im_MIP_4 = Im_med;
        Im_MIP_4 = Im_MIP_4>=ThreshMIP; 
        clear Im_med
        load([ROI_Folder,'\','Im_AlongDays_Week_4_',Animal_Name,'_MIP_sum']);
        Im_SIP_4 = Im_med;
        ...
        load([ROI_Folder,'\','Im_AlongDays_',Animal_Name,'_MIP_max']);
        Im_MIP = Im_med;
        Im_MIP = Im_MIP>=ThreshMIP;        
        clear Im_med
        load([ROI_Folder,'\','Im_AlongDays_',Animal_Name,'_MIP_sum']);
        Im_SIP = Im_med;
        Im_SIP = Im_SIP>=ThreshMIP;
        clear Im_med
