...
%%%%%%%%%%%%%%%%
if MeaMed == 1
    ISO_FOLDER  = [HD,':\LENS\Isocrone_SelSeq_MAX\Figures_Weekly_Mean of Trials\'];
elseif MeaMed == 2
    ISO_FOLDER  = [HD,':\LENS\Isocrone_SelSeq_MAX\Figures_Weekly_Median of Trials\'];
end
...
AREA_NAME   = {'M1 CFA','Motor','Sensory','CaudalAreas'};
...
% plot(DD_Matrix{i_T,1})    
    ...
    title(['Delay'])
%     xlim([0.5 11.5])
    set(gca,'xTick',     1:length(AREA_NAME(1:end)))
    set(gca,'xTickLabel',AREA_NAME(1:end))
%     ylim([0.1 1.1])
%     set(gca,'yTick',     0.1:0.1:1)
    ylabel('Delay Max{Area} - Max{Force}')
    xlabel('Area')
