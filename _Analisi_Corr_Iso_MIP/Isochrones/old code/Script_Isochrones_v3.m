...
%     N_reg     = 1024;                 %total number of region
    N_reg     = 16;                 %total number of region
...
%                             figure(FF)
                            ...
                            if all(~CheckSeq)
                                ...
                                choice = menu('it is ok?','Y','NO');
                                
                                if choice
                                    
                                    [x_SeqF y_SeqF] = ginput;
                                    StartFluoPeak = round(x_SeqF);
                                    close
...
%                 ImDelay_Day = nanmedian(ImDelay,3);
                ...
                colorbar
                caxis([-0.3 0.3])
                saveas(gca,strcat(Animal_name,'_',nday,'_','IsoChronesFig_ManChoice',num2str(N_reg)),'fig')
                saveas(gca,strcat(Animal_name,'_',nday,'_','IsoChronesFig_ManChoice',num2str(N_reg)),'jpeg')
                close all                
