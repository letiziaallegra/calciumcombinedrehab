%
% script to extract sequences and make isochrones
% 
...
for nd_i=3:length(NumDaysFolder)
    ...
    for fi=IntervalToFluoMean(1):IntervalToFluoMean(2)
        ...
        Ref_ImageTranslFilename = RealDay;
        Ref_Image_Dir           = dir([RotImageREF_Folder,'\',Animal_Name_choice,'_REF']);
        for rid=3:length(Ref_Image_Dir)
            if strfind(Ref_Image_Dir(rid,1).name,TrialDay)
                i_day_actual = rid-2;
                Ref_Image16 = imread([RotImageREF_Folder,'\',Animal_Name_choice,'_REF','\',Ref_ImageTranslFilename,'.tif']);
            end
        end
    ...
    Folder_Seq_Name = [MainDir,'\',DayCurrDir,'\','Sequence'];
