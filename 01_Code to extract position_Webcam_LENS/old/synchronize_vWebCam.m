function [res newfilename] = synchronize_vWebCam(filenamePOS_RAW,filenameFORCE_RAW, path,targetPos)
...
%prima filtro il segnale del LED
res.LEDgrayscale = sgolayfilt(double(res.LEDgrayscale),3,11);
mx = max(res.LEDgrayscale);
upperth = find(res.LEDgrayscale>mx-30); %<-- eventualmente cambiare (default mx-5)
...
upperth = upperth(upperth>indexPos0(end));
...
%     t = 0:Ts:(len-1)*Ts;
    t = Ts:Ts:(len-1)*Ts;
elseif(delay<0)
    Pos_New_first = [1:abs(delay)]' * 0; %min(Pos);% 0 %Pos(1,1);
