...
%         IntervalForce_Sig_Long   = cheb2BPfilt(ForceSignal(  stF_real-(IntFluoToPlot(1))    :   stF_real+(IntFluoToPlot(2))   ),0.1,30,2,Fs);
        %first derivative Force interval
%         der_IntervalForce_Sig_Long = derivative(cheb2LPfilt(IntervalForce_Sig_Long,0.04);
        der_IntervalForce_Sig_Long = sgolayfilt(derivative(IntervalForce_Sig_Long,0.04),3,21);
       
        %Fluo interval
        IntervalFluoROI_Sig_Long = FluoROI_Sig(  stF_real-(IntFluoToPlot(1))    :   stF_real+(IntFluoToPlot(2))   );
...
%         der_IntervalFluoROI_Sig_Long = derivative(IntervalFluoROI_Sig_Long,0.04);
        ...
        if ~(BadForcePeak) &&  ~(BadFluoPeak) %check if peaks are ok (it means that the peaks must have a positive first derivative - not flat (as example GCamp10_stroke_day5)        
            ...
            %         der_IntervalForce_Sig_Long = cheb2LPfilt(der_IntervalForce_Sig_Long, 6, 3, Fs);
            %         der_IntervalForce_Sig_Long = sgolayfilt(der_IntervalForce_Sig_Long, 5,11);
            %         der_IntervalForce_Sig_Long = cheb2BPfilt(cheb2LPfilt(der_IntervalForce_Sig_Long,1,3,Fs), 1,10,3,Fs);
            %         der_IntervalForce_Sig_Long = cheb2BPfilt(cheb2LPfilt(der_IntervalForce_Sig_Long,1,3,Fs), 1,10,3,Fs);
            %         der_IntervalForce_Sig_Long = cheb2BPfilt(derivative(cheb2LPfilt(IntervalForce_Sig_Long,3,3,Fs),0.04), 1,10,3,Fs);
            ...
            [ParForce ParFluo] = ParFromCorrespondingForceFluoPeaks(IntervalForce_Sig_Long, IntervalFluoROI_Sig_Long,...
                ...
                stF_real, IntFluoToPlot, trial_peak, status_peak, Fs, 1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%% 4) Compute Parameters from the Robot   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
