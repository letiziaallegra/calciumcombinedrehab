...
%     filename = ['dataMouseGCamp_',Animal_Name,'_',DayCurrDir(1:2),'.mat'];
    ...
    if exist([AnimalCurrDir,'\',filename])
        ...
        Threshold_Speed  = mean(Speed)+std(Speed)*1;
        % 
...
%                 end
    ...
    %%% Save dataGCamp %%%
    %%%%%%
    if strfind( DayCurrDir, 'stroke_BoNT' )
        postr = strfind( DayCurrDir, 'stroke_BoNT' );
        dat   = DayCurrDir(postr+12:postr+13);
    elseif strfind( DayCurrDir, 'stroke' )
        postr = strfind( DayCurrDir, 'stroke' );
        dat   = DayCurrDir(postr+7:postr+8);
    elseif strfind( DayCurrDir, 'control' )
        postr = strfind( DayCurrDir, 'control' );
        dat   = DayCurrDir(postr+8:postr+9);
    end
    dataGCamp.Info.Date = dat;
    
%     filename_GCamp = ['dataMouseGCamp_',dataGCamp.Info.Name,'_',dataGCamp.Info.Date,'_Par'];
    filename_GCamp = [filename];
    save([AnimalCurrDir,'\',filename_GCamp],'dataGCamp')
